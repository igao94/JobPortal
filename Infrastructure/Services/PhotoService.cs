...
public class PhotoService(IWebHostEnvironment env,
    IHttpContextAccessor httpContextAccessor) : IPhotoService
{
    public async Task<PhotoUploadResult> UploadPhotoAsync(IFormFile file)
    {
        ...
        if (!validExtensions.Contains(extension))
            throw new ArgumentException($"Valid extenions are: {string.Join(", ", validExtensions)}.");
    ...
    public string DeletePhoto(string url)
    {
